...
namespace Communicator.App.Tests.Integration
{
    public class DependencyInjectionTests
    {
        ...
        public void ConfigureServices_RegistersAllRequiredServices()
        {
            ...
            var authFactory = provider.GetService<Func<AuthViewModel>>();
            Assert.NotNull(authFactory);
            Assert.NotNull(authFactory());

            var homeFactory = provider.GetService<Func<UserProfile, HomePageViewModel>>();
            ...
            var settingsFactory = provider.GetService<Func<UserProfile, SettingsViewModel>>();
            Assert.NotNull(settingsFactory);
            Assert.NotNull(settingsFactory(user));

            var meetingFactory = provider.GetService<Func<UserProfile, MeetingSession?, MeetingSessionViewModel>>();
        ...
        public void ViewModels_CanBeResolvedDirectly_WhenRuntimeDependenciesAreRegistered()
        {
            ...
            var session = new MeetingSession("test-meeting", "host", 0, SessionMode.CLASS);
