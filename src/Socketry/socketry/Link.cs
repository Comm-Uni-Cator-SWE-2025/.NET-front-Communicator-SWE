...
public class Link
{
    ...
    public Link(ISocket connectedChannel)
    {
        _clientChannel = connectedChannel;
        _clientChannel.ConfigureBlocking(false);
        _packets = new ConcurrentQueue<Packet>();
    }
    ...
    int ReadAndParsePackets()
    {
        ...
        while (currDatapos < len)
        {
            ...
            int toRead = Math.Min(_currentSocketPacket._bytesLeft, data.Length - currDatapos);
            _currentSocketPacket._content.Write(data, currDatapos, toRead);
            _currentSocketPacket._bytesLeft -= toRead;
            currDatapos += toRead;

            if (_currentSocketPacket._bytesLeft == 0)
            {
                _packets.Enqueue(Packet.Parse(_currentSocketPacket._content.ToArray()));
    ...
    public Packet GetPacket()
    {
        ...
        Packet packet = _packets.TryDequeue(out Packet? p) ? p : null;
